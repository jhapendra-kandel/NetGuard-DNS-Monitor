...
def handle_dns_request(data, addr, sock, log_queue, all_logs, domain_blocklist, ip_blacklist):
    ...
    try:
        # Parse DNS request FIRST to get query_name
        ...
        # NOW check cache after query_name is defined
        if query_name in dns_cache:
            cache_hits += 1
            # sock.sendto(dns_cache[query_name], addr)  # Uncomment later when implementing cache
        ...
        if len(all_logs) > 5000:
            del all_logs[:len(all_logs)-5000]  # Keep only the last 5000 entries
    ...
    try:
        ...
        while True:
            ...
            threading.Thread(
                ...
                args=(data, addr, sock, log_queue, all_logs, domain_blocklist, ip_blacklist),
                daemon=True
            ).start()
